...
def feature_ML_Predict():
    ...
    if 'Excel' == input_method:
        ...
        st.sidebar.download_button("Download template", pd.DataFrame([loaded_model[1]]).to_csv(sep = ';',header = False ,index=False), file_name="template.csv")
        ...
        try:
            st.write(loaded_model[0].predict(input_for_prediction))
            # warnings.formatwarning(message, category, filename, lineno, file, line)
